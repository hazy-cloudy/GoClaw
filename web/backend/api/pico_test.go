...
func TestHandleWebSocketProxy_RewritesPetAliasToPicoUpstream(t *testing.T) {
	configPath := filepath.Join(t.TempDir(), "config.json")
	h := NewHandler(configPath)
	handler := h.handleWebSocketProxy()
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/pico/ws" {
			t.Fatalf("upstream path = %q, want %q", r.URL.Path, "/pico/ws")
		}
		w.WriteHeader(http.StatusOK)
		_, _ = io.WriteString(w, "proxied")
	}))
	...
	gateway.pidData = &ppid.PidFileData{}
	gateway.picoToken = "cached-token"

	req := httptest.NewRequest(http.MethodGet, "/pet/ws?session_id=test-session", nil)
	req.Header.Set(protocolKey, tokenPrefix+"cached-token")
	rec := httptest.NewRecorder()
	handler(rec, req)
